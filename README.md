...
- **Relationship Fields.** A value keeps the decision it was committed with, and a Related Record created by an earlier Commit is reused (its ID), never created again, nor looked up again. A record that could not be created is tried again. A value new or changed in Fix & Retry goes through Resolution first, alone: the Importer sees only those values, and `findRelated` receives only them.
